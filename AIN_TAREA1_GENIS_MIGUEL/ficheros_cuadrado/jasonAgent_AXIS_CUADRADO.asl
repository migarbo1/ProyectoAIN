...
+!update_targets 
	...
	?my_position(X,Y,Z);
	if (N == 0){
		!add_task(task(9999, "TASK_GOTO_POSITION",MyName,pos(100,Y,100),""));
	}else{
		if (N == 1) {
			!add_task(task(9999, "TASK_GOTO_POSITION",MyName,pos(X+60,Y,Z),""));
		}
		if (N == 2) {
			!add_task(task(9999, "TASK_GOTO_POSITION",MyName,pos(X,Y,Z+60),""));
		}
		if (N == 3) {
			!add_task(task(9999, "TASK_GOTO_POSITION",MyName,pos(X-60,Y,Z),""));
		}
