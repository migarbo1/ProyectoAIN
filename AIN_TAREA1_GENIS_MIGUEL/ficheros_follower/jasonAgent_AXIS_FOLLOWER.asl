 ...
 */
+!perform_look_action 
    <-  ?debug(Mode); if (Mode<=2) {.println("+++++++++++++++++++++++++++++++++++++++++++++++");}
        ?fovObjects(FOVObjects);
        .length(FOVObjects, Length);
        ?debug(Mode); if (Mode<=2) {.println("EL NUMERO DE OBJETOS ES: ", Length);}
		-+following("false");
		+bucle(0);
        if (Length > 0) {
				?debug(Mode); if (Mode<=2) {.println("BUSCANDO AGENTES A LOS QUE SEGUIR.");}
				while (following("false") & bucle(X) & (X < Length)) {
					...
					?debug(Mode); if (Mode<=2) {.println("ANALIZANDO OBJETO: ", Object);}
					
					if (Type > 1000) {
						?debug(Mode); if (Mode<=2) {.println("HE ENCONTRADO UN OBJETO.");}
					} else {
						// Object may be an AXIS
						.nth(1, Object, Team);
						?my_formattedTeam(MyTeam);
			  
						if (Team == 200) { 
							.nth(0, Object, Numero);
							?debug(Mode); if (Mode<=2) {.println("SIGUIENDO AL UN COMPANERO: ", Numero);}
							+followed_angent(Object);
							-+following("true");
						}
	...
	?encontrado(En);
	if(Foll == "false"){
		?debug(Mode); if (Mode<=2) {.println("NO SIGO A NINGUN COMPANERO"); }
	}
    -bucle(_);
	if(Foll == "true"){
		?followed_angent(Target);
		.nth(6, Target, NewDestination);
		.nth(0, Target, Numero);
		+sigo_a(Numero);
		.my_name(MyName);
		?revisar(K);
		?debug(Mode); if (Mode<=2) {.println("SIGUIENDO A UN COMPANERO EN LA POSICION: ", NewDestination);}
