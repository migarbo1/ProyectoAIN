 ...
 *
 */
+!checkMedicAction(Salud)
<-  
    .println("Compurebo que es necesario ir");
    ?my_health(Hr);
    ?my_health_threshold(Ht);
    if (ocupado("false") & (iaimed("false") | Salud < 35)){
        .println("in my way to help with Health");
        -+medicAction(on);
    }else{
...
+cfmnuestro(X, Y, Z, Salud)[source(M)]
    <-
         // Soy medico y me han pedido ayuda
         .println("Recibi pidicion de vida de", M);
        !checkMedicAction(Salud);
        if (medicAction(on)) {
            .println("preparo las cosas para ayudar");
            ?my_position(Mx, My, Mz);
            Xn = (X + Mx)/2;
            Zn = (Z + Mz)/2;
            !add_task(task(9000,"TASK_GIVE_MEDICPAKS", M, pos(Xn, Y, Zn), ""));
            .concat("cfm_agree","(",Xn, ", ", Y, ", ", Zn, ")", Content);
...
<-
    ...
    if (Ar <= At) {
        ...
        .concat("cfanuestro(",X, ", ", Y, ", ", Z, ", ", Ar, ")", Content1);
...
+cfa_agree(X , Y, Z)[source(M)]
   <-
    .wait(500);
    !add_task(task(8000,"TASK_GOTO_POSITION", M, pos(X, Y, Z), ""));
    -+state(standing);
    -cfm_agree.  
...
+!init
   ...
   -+objective(ObjectiveX, ObjectiveY, ObjectiveZ - 20);
   -+ocupado("false");
   .  
