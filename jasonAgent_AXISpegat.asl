 ...
 *
 */
+!perform_injury_action: quadrat(0) & guatafoc("false")
		...
		if (iaimed("true")){
			...
			if (H>=45){ //SE QUEDa a matarse
				if (X1>=X) { //moures a la dreta
					if (Y1>=Y){//moures dalt
						!add_task(task(5000,"TASK_ATTACK",MiNombre,pos(X+10,Y,Z-10),""));
					}else{ // moures baix
						!add_task(task(5000,"TASK_ATTACK",MiNombre,pos(X+10,Y,Z+10),""));
					}
				}else{
					if(Y1>=Y){//moures dalt
						...
						!add_task(task(000,"TASK_ATTACK",MiNombre,pos(X-10,Y,Z+10),""));
					}
				}
				...
				if (X1>=X) { //moures a la dreta
					if (Y1>=Y){//moures dalt
						!add_task(task(9000,"TASK_GOTO_POSITION",MiNombre,pos(X+10,Y,Z-10),""));
					}else{ // moures baix
						!add_task(task(9000,"TASK_GOTO_POSITION",MiNombre,pos(X+10,Y,Z+10),""));
					}
				}else{
