        ...
        if (Length > 0) {
            ...
            while (aimed("false") & bucle(X) & (X < Length)) {
                ...
                if (Type > 1000) {
					?my_ammo(Ma);
					?my_health(Mh);
                    if(Type == 1001 & Mh < 80){ //1001 medipack ---- 1002 ammopack
...
/////////////////////////////////
+tell_me_pos[source(M)]
	<-
	  ?my_position(Tx,Ty,Tz);
	  .concat("pos_told(",Tx,",",Ty,",",Tz,")",Msg);
	  .send_msg_with_conversation_id(M,tell,Msg,"CFAR");
	  -tell_me_pos.

+pos_told(Tx,Ty,Tz)[source(M)]
	<-
	  !add_task(task(9999,"TASK_GOTO_POSITION",MyName,pos(Tx,Ty,Tz),_));
	  -pos_told.
    
+cfm_agree[source(M)] //venen a ajudar aixi que sobrevisc
   <- ?debug(Mode); if (Mode<=1) { .println("YOUR CODE FOR cfm_agree GOES HERE.")};
	  . 

+cfa_agree[source(M)] //venen a ajudar aixi que sobrevisc
   <- ?debug(Mode); if (Mode<=1) { .println("YOUR CODE FOR cfa_agree GOES HERE.")};
      !perform_injury_action; //suposem que angel ha fet que se meneje
	  ?my_position(Px,Py,Pz);
	  .concat("pos_told(",Px,",",Py,",",Pz,")",Msg);
	  .send_msg_with_conversation_id(M,tell,Msg,_); //li diu la nova posició a la que s'ha menejat
      -cfa_agree.  

+cfm_refuse[source(M)] //m'han abandonat a la meua aixina que vaig a yolo
   <- ?debug(Mode); if (Mode<=1) { .println("YOUR CODE FOR cfm_refuse GOES HERE.")};
   	  !get_agent_to_aim;
	  .my_team("medic_AXIS", Agents);
	  !nearest(Agents);
	  ?nearest(Agent, Position,_);
	  !add_task(task(9999,"TASK_GOTO_POSITION",MyName,Position,""));
      -cfm_refuse.  

+cfa_refuse[source(M)]//el fieldops no pot vindre, vaig jo
   <- ?debug(Mode); if (Mode<=1) { .println("YOUR CODE FOR cfa_refuse GOES HERE.")};
      .concat("tell_me_pos",Msg);
	  .send_msg_with_conversation_id(M,tell,Msg,"CFAR");
      -cfa_refuse.  
